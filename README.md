...
**생성 코드, 검증 코드, 전 과정 기록, 샘플**. 데이터 자체(~620 GB)는 여기 없다.

| | replica | mp3d | gibson | s3d |
|---|---|---|---|---|
| 씬 | 17 | 159 층 / 83 건물 | 945 층 / 489 건물 | 700 |
| split | 11 / 3 / 3 | 131 / 16 / 12 | 795 / 81 / 69 | 400 / 50 / 250 |
| 프레임 (collection당) | 20,400 | 65,780 | 246,212 | 15,872 |
| RIR | ring + binaural, 2조건 | 동일 | ring, 2조건 | ring + binaural, 도면 조건 |
| 시멘틱 도면 | O | O | X | O |
...
- **[echoloc_dataset/](echoloc_dataset/)** — 데이터셋 루트에 두는 문서: 스펙, 4종 비교 README, 데이터셋별 README와 `dataset_meta.json`.
