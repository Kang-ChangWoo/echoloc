# echoloc_dataset — F3Loc 호환 floorplan localization 데이터셋 5종 (+ RIR)

`dataset_generation_spec.md`(F3Loc + AV-FPLoc 음향 확장)를 다섯 소스에 적용한 결과물.
...
└── zind/       ZInD 2,443 층씬 (1,575 집)          54 GB
```

데이터셋 폴더 하나의 내부 구조 (전부 동일):
...
|---|---|---|---|---|---|
| 씬 | 17 | 159 층씬 (83 건물) | 945 층씬 (489 건물) | 700 | 2,443 층씬 (1,575 집) |
| split (train/val/test) | 11 / 3 / 3 | 131 / 16 / 12 | 795 / 81 / 69 | 400 / 50 / 250 | 1,948 / 243 / 252 |
...
| 프레임 | 20,400 ×2 | 65,780 ×2 | 246,212 ×2 | 15,872 | 55,926 |
| 카메라 | 480×640, HFOV 106.3°, F_W 3/8 | 동일 | 동일 | 640×360, HFOV 80°, **F_W 0.596** | 동일 (파노라마 크롭) |
| 포즈 | navmesh 샘플링 | navmesh 샘플링 (층별) | navmesh 샘플링 (층별) | S3D 고정 카메라 위치 그대로 | ZInD 파노라마 위치 그대로 |
| 음향 조건 | raw_scan_open + floorplan_closed | 동일 | 동일 | **floorplan_closed만** (메시 없음) | **floorplan_closed만**, ring RIR만 |
| RGB 출처 | habitat 렌더 (Replica 메시) | habitat 렌더 (MP3D 메시) | habitat 렌더 (Gibson 메시) | S3D 제공 렌더 (가구 포함) | ZInD 파노라마 크롭 (실제 사진, 가구 포함) |
| 시멘틱 지도 | O | O | **X** (원본에 주석 없음) | O | O (문·창 주석) |

**F_W 주의**: s3d는 카메라 화각이 달라 `F_W = 0.596`이다. F3Loc config의 `F_W`를 데이터셋에 맞게 바꿔야 한다. zind는 3/8이지만 카메라 높이가 프레임마다 다르다(`chunks.json`).
...
- **depth_radial_\***: 480×640(s3d는 640×360) **픽셀 radial depth**, 16-bit PNG mm, 0 = 미충돌. `_scan`은 실제 메시(또는 S3D 렌더; zind는 없음), `_floorplan`은 프록시.
- **desdf**: `{"l","t","desdf"}`, 0.1 m/cell, 36 yaw bin, 10 m, float32.
- **RIR**: 48 kHz, indirect 20,000 rays·반사 50회·회절 on, 6ch 링(반경 5 cm, rel 0) + binaural(rel 0/90/180/270; gibson·zind는 링만). 음원은 수신기와 같은 위치.
...
root = "replica"          # or "mp3d", "gibson", "s3d", "zind"
